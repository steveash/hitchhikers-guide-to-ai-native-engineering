...
### Claim 8: Beyond the httpx2 compatibility fix, llm-anthropic 0.27 also shipped six unrelated bug fixes and feature additions not mentioned in the blog post
...
- **Our assessment**: The blog post's "mainly provides compatibility" framing (Claim 1) is accurate but incomplete as a full changelog. The release notes carry seven bullets; the blog post discusses only the first (`anthropic>=1`, #84). The other six, in release-note order: (1) `--no-stream` now uses the streaming API under the hood, fixing an SDK error ("Streaming is required for operations that may take longer than 10 minutes") for models with large default `max_tokens` (#85); (2) a `TypeError` when `temperature=None` was passed explicitly, plus a validation bug where setting only `top_p` raised "Only one of temperature and top_p can be set" (#70, credited to external contributor Charlie Tonneslan); (3) `-o top_p 0.0` now sent to the API rather than treated as unset (#74, also Charlie Tonneslan); (4) structured-outputs support for schemas on Claude Haiku 4.5 (#61); (5) mid-conversation system-message support for Claude Opus 4.8 and the Claude 5 family (#73); (6) preservation and tool-continuation replay of thinking blocks with omitted content and `redacted_thinking` blocks (#81). Practitioners relying solely on the blog post (rather than the full release notes) would miss all six — including #85, which is a user-visible behavioural change to an existing CLI flag rather than a purely internal fix. This is a general lesson about Willison's release-announcement format: the "beat" post highlights the single most structurally significant change, not a complete changelog — consistent with the same pattern already noted for the 0.25.1 and 0.26 announcements in this corpus (see Cross-References → Extends).
...
- **Corroborates**:
  - `blog-simonwillison-llm-anthropic-0251.md` and `blog-simonwillison-llm032.md`: both prior `llm-anthropic` release notes are structured identically to this one — a short first-party "beat" post with verbatim release-note bullets, model/version-specific changes, and (for 0251) linked-page context the Miner is expected to follow. This note's Claim 8 (blog post omits six of seven release-note items) generalizes a pattern implicit but not stated in those earlier notes: Willison's release "beats" highlight the single most narratively interesting change, not a full changelog.
