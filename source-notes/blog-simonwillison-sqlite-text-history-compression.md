...
- **Confidence**: emerging (one trial, one machine, one workload — `REPORT.md`'s Workload section states "The focused 1,000-edit monolithic run was one trial because it is deliberately expensive;", so these specific figures rest on a single run; the three-trial medians belong to the chunked results in Claim 3, not to this monolithic design — but the mechanism, decompress-append-recompress on every write, is a straightforward and predictable cause of the measured quadratic behavior)
...
- **Confidence**: emerging (same one-machine, one-synthetic-workload caveat as Claim 2, but on a firmer statistical footing than it: per `REPORT.md`'s Workload section the 64- and 128-revision chunk figures are medians from three trials, whereas the monolithic figures they are compared against come from a single run)
- **Quote**: "A chunk size of 128 retains most of the monolithic compression ratio: its compressed history is only about 30 KB larger." — *(`REPORT.md`, "Focused results at 1,000 edits"; verified character-for-character against the raw file)*
...
- **Our assessment**: Consistent with Zstandard's known advantages over zlib for both ratio and speed on general text, and `REPORT.md`'s own recommendation reflects it directly: "Use Zstandard rather than zlib, keep the current text outside the blob, append to canonical serialized JSON without parsing it, and update everything in one transaction." (`README.md`'s ordered list under "Zstandard availability" is about which *Zstandard implementation* to reach for — `compression.zstd`, then the `zstandard` package, then a `ctypes` wrapper around `libzstd` — not about zstd-vs-zlib, so it is not evidence for this claim.) Not a surprising result, but useful as a concrete, measured data point rather than a general assertion.
...
- **Evidence**: `REPORT.md`'s "Practical recommendations" section splits into two bulleted lists under the headings "Use the one-BLOB design when" and "Use chunked JSON when". The one-BLOB list is conditioned on "histories are normally a few hundred revisions" and "edits are relatively infrequent"; the chunked list on "saving on every keystroke or autosave interval", "documents can accumulate thousands of versions", and "memory and write amplification need a hard bound". Each fragment is a separate contiguous bullet, not one continuous sentence.
...
- **Quote**: "For that case, the original proposal is not merely plausible—it is very good." — *(`REPORT.md`, closing the "Use the one-BLOB design when" list; verified character-for-character against the raw file, em dash unspaced as in the original)*
...
- **Evidence**: A separate sensitivity analysis (`sensitivity.py` in the repo, results summarized in `REPORT.md`) varying the percentage of a document changed per edit; storage grows "roughly with the amount of new information introduced by edits". `REPORT.md`'s sensitivity table shows the collapse concretely: with high-entropy content and only 20 bytes replaced per edit the monolithic blob is still 45.8 KB, but at 10% replaced per edit it is 1.6 MB, and for fully independent high-entropy snapshots all three strategies converge on 16.0 MB against 19.1 MB of raw snapshots — i.e. no better than ordinary per-snapshot compression.
- **Confidence**: emerging (one synthetic sensitivity sweep)
- **Quote**: "There is no magic when successive versions are unrelated; the scheme then approaches ordinary snapshot storage." — *(`REPORT.md`, "Sensitivity to the amount of change"; verified character-for-character against the raw file)*
...
- **Evidence**: `README.md`'s "Important semantic choices" list: current version is not duplicated in history; replacement atomically appends the old current value to history before installing the new value; `BEGIN IMMEDIATE` serializes writers. `REPORT.md`'s production-details list offers a second option for the same hazard: "Use `BEGIN IMMEDIATE`, or an optimistic `UPDATE ... WHERE revision_count = ?`, to prevent two writers from losing a revision."
...
- **Quote**: "`BEGIN IMMEDIATE` serializes writers to avoid losing a revision." — *(final bullet of `README.md`'s "Important semantic choices" list; verified character-for-character against the raw file at `raw.githubusercontent.com/simonw/research/main/sqlite-text-history-prototype/README.md`)*
...
`simonw/research/tree/main/sqlite-text-history-prototype` ("Focused results at
1,000 edits" table, checked figure-by-figure against the raw file — see
Extraction Notes):
...
- Quote provenance, revised: the original draft of this note was extracted
  through a fetch-and-summarize tool that renders pages through a secondary
  model rather than returning raw source, and it visibly paraphrased the same
  underlying sentence differently across calls. Several `Quote` fields were
  therefore marked unconfirmed inline or left empty. On rework I retrieved
  `README.md` and `REPORT.md` as raw text directly from
  `raw.githubusercontent.com/simonw/research/main/sqlite-text-history-prototype/`
  and re-checked every quoted string in this note character-for-character
  against those files. Three quotes changed as a result: Claim 8's
  `BEGIN IMMEDIATE` line (the earlier "to prevent revision loss" wording was a
  blend of `README.md`'s "to avoid losing a revision" and `REPORT.md`'s
  separate production-details bullet), Claim 3's chunk-size sentence, and the
  quoted fragments in Claim 5's Evidence, which had spliced three separate
  bullets of `REPORT.md`'s chunked-recommendation list into one continuous
  quoted string. Claims 5 and 6, previously "(no direct quote)", now carry
  verified verbatim quotes. Claim 1's and Claim 7's quotes come from the blog
  post itself and were unchanged.
- The benchmark tables in Concrete Artifacts were re-verified figure-by-figure
  against `REPORT.md`'s "Focused results at 1,000 edits" and "Storage result"
  tables, and the two Python usage examples against `README.md`. All match.
- Trial counts differ by row and matter for how much weight each figure
  carries: `REPORT.md`'s Workload section states that the 300-edit comparison
  and the 64-/128-revision JSON chunk results are medians from three trials,
  while the focused 1,000-edit monolithic run was a single trial "because it
  is deliberately expensive". Claim 2's confidence grade was corrected to
  reflect this; Claim 3's three-trial basis is the stronger of the two.
