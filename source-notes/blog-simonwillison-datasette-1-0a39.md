...
### Claim 6: Fixes were developed on the main branch first, then a strict subset was backported to the 0.65.x stable line, so both releases could ship the same day
- **Evidence**: Direct first-party statement, independently corroborated by diffing the two changelogs: the 1.0a39 changelog lists 18 distinct security-fix bullets; the 0.65.4 changelog lists 6 of those (case-insensitive permission names, `?_through=` permission, SQL identifier escaping for primary-key columns, FTS parameterization, `Cache-Control` tightening, and extension-loading lockdown) — a proper subset, not the full list.
