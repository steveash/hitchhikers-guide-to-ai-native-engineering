...
> Cursor's iOS public-beta announcement's most genuinely new contribution to the corpus is bidirectional local↔cloud agent migration presented as a named user-facing behavior — moving an active local agent to the cloud to keep running, and moving a cloud session back to the computer to test locally before merging. It also extends two already-emerging patterns onto a mobile surface: screenshot-as-agent-visual-context (established in `blog-cursor-design-mode.md`, here in a mobile-native, external-feedback-sourced variant) and voice-as-agent-invocation (established in `docs-github-copilot-cli-rubber-duck-scheduling-voice.md` for desktop CLI, here as phone-microphone-to-agent-prompt). And it provides dual-vendor confirmation (alongside `docs-github-copilot-cli-remote-control-ga.md`) that multi-platform async agent control is a converging convention across major AI coding tools.
...
- **Our assessment**: Voice input is the most structurally significant of the three: it converts spoken task descriptions into agent prompts, removing the text-entry friction of mobile keyboards for long-form task specifications. This is a second corpus instance of an already-emerging pattern rather than a first: `docs-github-copilot-cli-rubber-duck-scheduling-voice.md` Claims 4–6 document GitHub Copilot CLI voice input (GA, June 2026) as an on-device dictation modality for describing agent instructions (spacebar-hold or Ctrl+X+V, with all audio processed locally). The two instances differ in surface and privacy posture: Copilot CLI is desktop-terminal dictation with an explicit local-processing guarantee and a one-time runtime download; Cursor's is mobile voice-to-agent-prompt where the microphone is the phone's native input surface (the source makes no local-vs-cloud processing claim). Together they establish voice as a converging cross-tool agent-invocation modality across both desktop-CLI and mobile surfaces. Slash commands surface the same directive vocabulary as the desktop (e.g., `/fix`, `/explain`) without requiring full-form typing. Frontier model selection on mobile confirms capability parity with the desktop is a design goal — users are not limited to a constrained mobile model tier.
...
### Claim 4: The handoff pattern is bidirectional — active agent plans/sessions can move from local to cloud execution mid-workflow, and cloud sessions can be moved back to the local machine to test changes before merging

- **Evidence**: Explicit product description of both handoff directions.
- **Confidence**: anecdotal (vendor claim; no practitioner report of this capability in use)
- **Quote**: "send a local plan to a cloud agent or move active agents to the cloud to keep running." And: "You can move the cloud session back to your computer to test changes locally before merging."
- **Our assessment**: This is the most architecturally significant claim in the source. The handoff is bidirectional: an active agent can migrate local→cloud (to keep running after the developer closes their laptop) *and* cloud→local (to bring a cloud session back to the desktop for local testing before merging). Both directions are only possible because of the three-component decoupling architecture described in `blog-cursor-cloud-agent-lessons.md` Claim 6 ("An agent might run on one machine, spawn async subagents across several, or start locally then delegate work to the cloud"). The iOS post is the first corpus source to present this as a named user-facing product behavior rather than an architectural capability. The practical use cases: a developer starts a complex task locally, realizes it will run for hours, and migrates it to cloud execution so it continues after they close their laptop; conversely, a developer who kicked off a long cloud run pulls the session back to their machine to verify the changes locally before merging — the cloud→local return path closes the loop between async background work and local verification.
...
- **Our assessment**: Screenshot annotation is the only genuinely mobile-native input pattern in this source — it exploits the phone's natural position as the device where developers consume social media and user feedback. The workflow: see feedback on X → screenshot → annotate → send to agent as visual context → agent begins UI/design work. Screenshots-as-agent-visual-context is not itself new to the corpus: `blog-cursor-design-mode.md` Claim 2 already documents a screenshot as a spatial-context signal for the agent ("a screenshot for spatial context (layout, surrounding elements, and the exact page state)... This gives the agent exactly what it needs to find the source and edit the code efficiently"), and its Claim 3 documents drawing/annotation over a frozen viewport frame. What is genuinely new here is the *mobile-native, external-feedback-sourced* variant: the screenshot originates from social media/user feedback consumed on the phone (not a live click-captured frame of the developer's own running app), is annotated on the phone, and is dispatched to a remote agent — versus design-mode's in-IDE click-to-select-element flow. Note also that `blog-cursor-canvas.md` covers agents *producing* visual artifacts; this source, like design-mode, concerns visual *input* to agents — but sourced from the phone.
...
HANDOFF (bidirectional):
  local -> cloud: "send a local plan to a cloud agent or move active agents
                   to the cloud to keep running."
  cloud -> local: "You can move the cloud session back to your computer to
...
- **Extends**: `docs-github-copilot-cli-remote-control-ga.md` Claim 2 — GitHub's remote control enumerates six specific async interaction types (track progress live, steer or queue messages, review/tweak plans, stop sessions, approve/deny permissions, respond to questions). Cursor's iOS app extends the underlying pattern in three ways not present in the GitHub note: (1) launching new cloud agent sessions from mobile (not just controlling existing ones), (2) bidirectional local↔cloud agent migration mid-session, and (3) multimodal input via screenshot annotation with markup.

- **Corroborates / distinguishes**: `blog-cursor-design-mode.md` Claims 2–3 — design-mode already establishes screenshots as agent visual context (Claim 2: "a screenshot for spatial context (layout, surrounding elements, and the exact page state)... This gives the agent exactly what it needs to find the source and edit the code efficiently") and annotation over a frozen viewport frame (Claim 3). The iOS source is therefore *not* the first corpus documentation of screenshot-as-agent-context. It contributes a distinct variant: the screenshot is externally sourced (user feedback consumed on the phone, e.g. from X) rather than click-captured from the developer's own running app, and the whole capture→annotate→dispatch loop happens mobile-natively. Cross-referenced so the two visual-input patterns are compared rather than conflated.

- **Corroborates / distinguishes**: `docs-github-copilot-cli-rubber-duck-scheduling-voice.md` Claims 4–6 — Copilot CLI voice input (GA, June 2026) is a prior corpus instance of voice as an agent-instruction modality: on-device dictation (spacebar-hold or Ctrl+X+V) with an explicit local-audio-processing guarantee (Claim 5) and a one-time runtime download (Claim 6). Cursor's mobile voice input is a second instance on a different surface (phone microphone → agent prompt, with no stated processing-locality claim). Together they make voice a converging cross-tool invocation modality across desktop-CLI and mobile.

- **Novel**: The following claims are not documented in any other source note:
  - **Local-to-cloud agent migration as a named user-facing operation, and the cloud-to-local return path**: The architectural possibility of starting locally and delegating to the cloud was documented in `blog-cursor-cloud-agent-lessons.md` Claim 6; this is the first corpus source to present migration as a named, user-invocable product behavior *and* the first to document the bidirectional cloud→local return path ("move the cloud session back to your computer to test changes locally before merging").
  ...
  - **Mobile-native, external-feedback-sourced screenshot input**: While screenshots-as-agent-context are established (`blog-cursor-design-mode.md` Claims 2–3), the specific mobile variant — screenshotting external user feedback consumed on the phone, annotating it there, and dispatching to a remote agent — is new. Framed here as a variant, not a first, per the cross-reference above.
  - **Voice as a *mobile* agent-invocation surface**: Voice-as-instruction is not new to the corpus (see the Copilot CLI cross-reference above), but the phone microphone as the voice-to-agent-prompt surface, and voice as a friction reducer specifically for mobile long-form task specification, is the mobile-surface extension of that emerging pattern.
...
- **Chapter 01 (Daily Workflows — multimodal agent input)**: Claim 8 (screenshot annotation) should be added to any discussion of agent task specification techniques, alongside the existing visual-input coverage in `blog-cursor-design-mode.md` (Claims 2–3, in-IDE element screenshots). Together they establish visual context — provided by the developer rather than generated by tooling — as a distinct input class relevant for UI, design, and front-end work; the iOS source adds the mobile-native, external-feedback-sourced variant. This complements text-based prompting and CLAUDE.md context engineering, not replaces them.
...
- The Prospector's triage correctly identifies this as low-to-medium novelty marketing content. The clearest genuinely-new addition is bidirectional local↔cloud agent migration presented as a named user-facing behavior (including the cloud→local return path); the screenshot-as-input and voice-as-invocation contributions are mobile-surface *variants* of patterns already in the corpus (`blog-cursor-design-mode.md` Claims 2–3 and `docs-github-copilot-cli-rubber-duck-scheduling-voice.md` Claims 4–6 respectively), not first coverage — see Cross-References. The source provides no engineering depth for any of them.
