...
- **Evidence**: The changelog's "Configuration Options" section (Dashboard and CLI subsections) and CLI command example, corroborated and extended with parameter constraints on the linked reference page.
...
- **Evidence**: A dedicated reset-schedule table on the linked reference page, expanding on the changelog's briefer "resets occurring at UTC window starts" line.
- **Confidence**: settled (first-party documentation of a specific, deterministic scheduling behavior)
- **Quote**: (table data extracted verbatim — see Concrete Artifacts; the changelog's own summary sentence is: "The refresh period options include `daily`, `weekly`, `monthly`, or `none`, with resets occurring at UTC window starts.")
...
- **Quote**: "A key's only editable property is its budget." / "Removing a budget archives it rather than deleting it, so you restore it with PATCH, not POST (a POST would return 409 because the record still exists)."
